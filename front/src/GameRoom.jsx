...
const GameRoom = ({ setCurrentPage}) => {  // <-- Ajout de setCurrentPage
    ...
    useEffect(() => {
        ...
        ws.current.onopen = async () => {
            ...
            if (ws.current && ws.current.readyState === WebSocket.OPEN) {
                ...
                setTimeout(500); // Réessaye après 500ms
            }
        };
        checkSession();
    ...
    useEffect(() => {
        console.log("User maj :", users);
    }, [users]);

    // Créer une room
    const createRoom = () => {
        if ( ws.current.readyState !== WebSocket.OPEN) {
            console.warn("WebSocket pas encore prêt, réessai dans 500ms...");
        ...
        // Appeler fetchUsersInRoom uniquement si le WebSocket est prêt
    };
